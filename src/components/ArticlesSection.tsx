...
export const ArticlesSection = () => (
  <Chapter id="writes" className="py-28 md:py-40">
    <div className="container-x">
      <div className="flex flex-wrap items-end justify-between gap-6">
        <ChapterIntro
          eyebrow="Writes · Substack"
          title={
            <>
        ...
        />
        <Reveal delay={0.2}>
          <a href="https://samarthsaraswat.substack.com/" target="_blank" rel="noopener noreferrer" className="btn-ghost">
        ...
        {articles.map((article, i) => (
          <Reveal key={article.title} delay={0.1 * i}>
            <a href={article.url} target="_blank" rel="noopener noreferrer" className="group block">
              ...
              <h3 className="mt-5 text-xl font-semibold leading-snug text-bone transition-colors group-hover:text-accent">
                {article.title}
              </h3>
              <p className="mt-2 text-[15px] leading-relaxed text-mist">{article.description}</p>
              <span className="mt-4 inline-flex items-center gap-1.5 text-[14px] font-medium text-bone-dim">
